...
NumberList::NumberList(const NumberList& copyObj) {
   ...
   //loops through all the elements of copyObj and copies there values to this list through the appendNode() function
...
NumberList& NumberList::operator=(const NumberList& copyObj) {
   //Checks to make sure we aren't sel-assign
   if(this != &copyObj) {
      ListNode *nodePtr;
      ListNode *nextPtr;
      nodePtr = this->head;

      //Frees up all the memory allocated by this list
