...
int main(int argc, char* argv[4]) {
    ...
    while (!hasFinishedGame) {
        hasFinishedGame = play_game();
    }

    cout << playerName << ", your final score was: " << score << endl;
    //Sorts and displays score
    sort_score();
...
bool play_game(){
    ...
    if (usedQuestions.size() >= 6) {
        cout << "You have reached the end of the game. Congratulations!" << endl;
        return true;
    }
    ...
    cout << playerName << " Here's Question Number " << usedQuestions.size() << endl;
    //Read player input
    ...
    //Else the player looses and we exit the function with a loosing exitcode
        ...
        cout << "Incorrect again! sorry you have lost the game" << endl;
        score = 0;
        return true;
    }
...
void sort_score() {
    ...
    cout << "Your score has been recoded." << endl;
    ...
    //Ouput the top record holder
    cout << "the current record holder is " << names.at(0) << " with a score of " << scores.at(0) << endl;
    
    //If the score is zero than by default they would be in last place
    if (score == 0)
    {
        cout << "You are at rank " << names.size() << " with a score of " << score << endl; 
    }
    
    //If the score is better than 0 then we linear search for there name in the sorted ranking
    else {
        for (int i = 0; i < numberOfLines; i++) {
            if (playerName == names.at(i))
            {
                cout << "You are at rank " << (i + 1) << " with a score of " << score << endl;
                break;
            }
